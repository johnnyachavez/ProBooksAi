...
- **`build_desktop.ps1`** / **`build_desktop.sh`** — PyInstaller onefile build (output **ProBooksPlusAi**). **`--add-data`** copies **`ai/`** (Document Intake **Run AI**), **`probooks/`**, **`probooksai/`**, **`desktop_app/`**, **`docs/`**, and root **`pyproject.toml`**; **`--hidden-import generate_workbook`** bundles root **`generate_workbook.py`** for COA seeding; **`--hidden-import openai`** and **`pypdf`** pull in lazy imports used by **`ai.extractor`** (install **`openai`** and **`pypdf`** before building, e.g. **`pip install -r requirements.txt`** or **`.[ci]`**). Run **`pip install -e ".[desktop]"`** first so **`--copy-metadata probooks-ai`** embeds wheel version metadata (**Help → About**); without it, **`desktop_app.version`** can still parse **`[project].version`** from the bundled **`pyproject.toml`**.
