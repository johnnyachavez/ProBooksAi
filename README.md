...
**File → Backup** / **File → Restore** use the same **`probooks.backup`** online-backup path as the CLI; see [Python CLI](#python-cli) (**Regression tests**, **#28**).

The app uses a **Fusion**-based dark theme (`desktop_app/theme.py`). The **menu bar** fills the **status bar** with hints when you hover **File**, **View**, **Edit**, **Tools**, **Recon**, and **Help** (`QAction.setStatusTip` in `desktop_app/main.py`). Main areas include **Document intake** (**F5** refresh inbox; **File** → **Import documents…** / **Ctrl+O**; **Help** → **Document intake shortcuts…** includes **View** **Ctrl+1–8** labels; inbox **right-click** **Keyboard shortcuts…**; **Run AI** / **Approve** / **Mark Posted** / **Reject** **tooltips**), **Bank import** (CSV + reconciliation; **F5** reload; **Help** / **right-click** **Keyboard shortcuts…** on the tab and **Manage Bank Accounts**; footer hint), **Journal** (**F5** refresh), **Business** (**F5** refresh Rules / AR / AP / Payroll on the active sub-tab; **Help** → **Business shortcuts…**; main grids and copyable modal tables **right-click** **Keyboard shortcuts…**), **COA** (**F5** refresh), **Reports** (**F5** re-run last report), **Audit** (**F5** refresh), **Help** → **More tab shortcuts (F5)…** (COA / Journal / Reports / Audit + grid **Keyboard shortcuts…**), and **Bank register** (grid-style table, chronological transactions per account, debit/credit, running balance, **Clr** column **C**/batch-**R** with header and row **tooltips**, filters including cleared and batch reconciliation, double-click **Clr** to toggle cleared, **F5** refresh, **Ctrl+Shift+G** post selected to GL, **Ctrl+Shift+C** / **Ctrl+Shift+U** mark cleared / clear cleared, **Ctrl+Shift+E** export CSV with Register focus, **Recon** menu **Register Actions** / **Reconciliation** / **Attachments** / **Transaction Tools** / **Flags** with **`QAction` tips** (same actions as the former on-tab buttons); **Tools** → **Invoice…** (**Ctrl+Shift+I**) opens **Business** → **Invoices (AR)**, **Help** → **Bank register keyboard shortcuts…** (cross-links **Bank import shortcuts…**) / **right-click** grid **Keyboard shortcuts…**, COA assignment, post-to-GL; **filter, last bank account, and column widths persist per company SQLite file** via `QSettings`) — see `desktop_app/bank_import_tab.py` and `desktop_app/register_tab.py`. On some **Windows** builds **Qt** may log a harmless **`QFont::setPointSize`** line at startup; `desktop_app/main.py` installs a narrow **Qt** message filter for that text before **`QApplication`** is constructed. Contract coverage: `tests/test_desktop_main_contract.py` and **CONTRIBUTING.md** (contract tests table).
...
- **`ci_validate_layout.sh`** / **`ci_validate_layout.ps1`** — Same required-path checks as the CI **validate** job. From the repo root: `bash scripts/ci_validate_layout.sh` in **Git Bash**, **WSL**, or another shell where **`bash`** is on **`PATH`** (plain **Windows PowerShell** often has no **`bash`** on **`PATH`**; use **`.\scripts\ci_validate_layout.ps1`** for the same checks). Keep **both** files in sync when you add or reorder **`require`** paths (hub HTML, contract tests, **`tests/conftest.py`**, **README**, **`.gitattributes`**, …); **`.github/workflows/ci.yml`** invokes the **`.sh`** script on Linux runners. Root **`.gitattributes`** keeps **`*.yml`**, **`*.yaml`**, and **`*.sh`** checked out as LF for stable diffs with Linux CI.
- **`build_desktop.ps1`** / **`build_desktop.sh`** — PyInstaller onefile build (output **ProBooksPlusAi**). **`--add-data`** copies **`ai/`** (Document Intake **Run AI**), **`probooks/`**, **`probooksai/`**, **`desktop_app/`**, **`docs/`**, and root **`pyproject.toml`**; **`--hidden-import generate_workbook`** bundles root **`generate_workbook.py`** for COA seeding; **`--collect-submodules openai`**, **`pydantic`**, **`httpx`**, **`httpcore`**, **`h11`**, and **`anyio`** bundle the OpenAI HTTP client stack (lazy-loaded from **`ai.extractor`**); **`--hidden-import pypdf`** covers PDF text (**install dependencies before building**, e.g. **`pip install -r requirements.txt`** or **`.[ci]`**). Run **`pip install -e ".[desktop]"`** first so **`--copy-metadata probooks-ai`** embeds wheel version metadata (**Help → About**); without it, **`desktop_app.version`** can still parse **`[project].version`** from the bundled **`pyproject.toml`**.
- **`sync-workspace.ps1`** — Optional local snapshot of repo + GitHub issues/PRs into **`integrations/work-context.json`** (requires [GitHub CLI](https://cli.github.com/) and `gh auth login`). The committed **`integrations/work-context.example.json`** documents the minimal shape: exactly four **`localWorkFiles`** paths — **`index.html`**, **`invoice.html`**, **`review.html`**, **`docs/ROADMAP.md`** — enforced by **`tests/test_integrations_example_contract.py`**. Generated JSON lists every repo file and may add extra PR/issue fields from **`gh`** (see script **`.DESCRIPTION`**). **Troubleshooting:** [Running Tests](docs/CONTRIBUTING.md#running-tests) (**`gh auth login`**, **`gh repo set-default`**).
- **`capture_ui_screenshot.py`** — Headless capture of the main window on the **Invoices** tab plus **Create Invoices** (`invoice_create.png`, `invoice_create_header.png`; see script docstring); writes under **`artifacts/`** (gitignored). Pull requests also trigger **`.github/workflows/ui-screenshot.yml`** (non-blocking; posts a comment with the artifact link).
