...
The app uses a **Fusion**-based dark theme (`desktop_app/theme.py`). The **menu bar** fills the **status bar** with hints when you hover **File**, **View**, **Edit**, **Help**, and **Tools** (`QAction.setStatusTip` in `desktop_app/main.py`). Main areas include **Document intake** (**F5** refresh inbox; **File** → **Import documents…** / **Ctrl+O**; **Help** → **Document intake shortcuts…** includes **View** **Ctrl+1–8** labels; inbox **right-click** **Keyboard shortcuts…**; **Run AI** / **Approve** / **Mark Posted** / **Reject** **tooltips**), **Bank import** (CSV + reconciliation; **F5** reload; **Help** / **right-click** **Keyboard shortcuts…** on the tab and **Manage Bank Accounts**; footer hint), **Journal** (**F5** refresh), **Business** (**F5** refresh Rules / AR / AP / Payroll on the active sub-tab; **Help** → **Business shortcuts…**; main grids and copyable modal tables **right-click** **Keyboard shortcuts…**), **COA** (**F5** refresh), **Reports** (**F5** re-run last report), **Audit** (**F5** refresh), **Help** → **More tab shortcuts (F5)…** (COA / Journal / Reports / Audit + grid **Keyboard shortcuts…**), and **Bank register** (grid-style table, chronological transactions per account, debit/credit, running balance, **Clr** column **C**/batch-**R** with header and row **tooltips**, filters including cleared and batch reconciliation, double-click **Clr** to toggle cleared, **F5** refresh, **Ctrl+Shift+G** post selected to GL, **Ctrl+Shift+C** / **Ctrl+Shift+U** mark cleared / clear cleared, **Ctrl+Shift+E** export CSV with Register focus, **Tools** menu **Register Actions** / **Reconciliation** / **Attachments** / **Transaction Tools** / **Flags** with **`QAction` tips** (same actions as the former on-tab buttons), **Help** → **Bank register keyboard shortcuts…** (cross-links **Bank import shortcuts…**) / **right-click** grid **Keyboard shortcuts…**, COA assignment, post-to-GL; **filter, last bank account, and column widths persist per company SQLite file** via `QSettings`) — see `desktop_app/bank_import_tab.py` and `desktop_app/register_tab.py`. On some **Windows** builds **Qt** may log a harmless **`QFont::setPointSize`** line at startup; `desktop_app/main.py` installs a narrow **Qt** message filter for that text before **`QApplication`** is constructed. Contract coverage: `tests/test_desktop_main_contract.py` and **CONTRIBUTING.md** (contract tests table).
