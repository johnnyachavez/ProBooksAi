...
- **`build_desktop.ps1`** / **`build_desktop.sh`** — PyInstaller onefile build (output **ProBooksPlusAi**). **`--add-data`** copies **`probooks/`**, **`probooksai/`**, **`desktop_app/`**, **`docs/`**, and root **`pyproject.toml`**. Run **`pip install -e ".[desktop]"`** first so **`--copy-metadata probooks-ai`** embeds wheel version metadata (**Help → About**); without it, **`desktop_app.version`** can still parse **`[project].version`** from the bundled **`pyproject.toml`**.
