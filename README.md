...
- **`build_desktop.ps1`** / **`build_desktop.sh`** — PyInstaller build for the desktop app (output name **ProBooksPlusAi**). Run **`pip install -e ".[desktop]"`** first so **`--copy-metadata probooks-ai`** embeds package version metadata in the frozen app (same string as **Help → About**). **`pyproject.toml`** is also bundled at the extract root so **`desktop_app.version`** can still read **`[project].version`** if metadata is missing.
