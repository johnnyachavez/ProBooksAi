...
- **`build_desktop.ps1`** / **`build_desktop.sh`** — PyInstaller onefile build (output **ProBooksPlusAi**). **`--add-data`** copies **`ai/`** (Document Intake **Run AI**), **`probooks/`**, **`probooksai/`**, **`desktop_app/`**, **`docs/`**, and root **`pyproject.toml`**; **`--hidden-import generate_workbook`** bundles root **`generate_workbook.py`** so **`probooksai.coa_db.seed_from_workbook`** can load **`COA_DATA`** in the frozen app. Run **`pip install -e ".[desktop]"`** first so **`--copy-metadata probooks-ai`** embeds wheel version metadata (**Help → About**); without it, **`desktop_app.version`** can still parse **`[project].version`** from the bundled **`pyproject.toml`**.
