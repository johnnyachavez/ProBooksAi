...
from probooksai.company_identity import company_identity_print_fields
...
def invoice_html_string(conn: sqlite3.Connection, invoice_id: int) -> str:
    ...
    cid = company_identity_print_fields(conn)

    return build_invoice_print_html(
        company_name=cid["name"],
        company_address=cid["address"],
        company_phone=cid["phone"],
        company_email=cid["email"],
        company_tax_id=cid["tax_id"],
