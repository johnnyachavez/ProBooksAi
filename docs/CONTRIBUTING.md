...
| **`test_package_name_contract.py`** | **`probooks-ai`** in **`desktop_app/version.py`**, **`probooks/__init__.py`** ( **`help_epilog`** in package docstring); **`probooks/help_epilog.py`** module doc ↔ **`probooksai.generator`** |
