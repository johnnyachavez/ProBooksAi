...
- **README `### Default database paths (Windows)`** — Keep **README** default-path bullets aligned with **ROADMAP** implementation snapshot (**Why not one `.db` yet**, **#21**, two filenames); **`review.html`** Documentation card + **Python + desktop** paragraph link; **`config.yml`** **Default database paths** contact; **`test_readme_default_database_paths_notes_two_schemas_and_roadmap`**, **`test_review_html_links_readme_web_shell_and_desktop_anchors`**, **`test_review_html_readme_default_database_paths_documentation_card`**, **`test_review_html_python_desktop_section_mentions_help_epilog`**, **`test_pr_template_lists_readme_default_database_paths_checklist`**, **`test_contributing_ci_documents_readme_default_database_paths_bullet`** in **`test_ci_validate_layout_sync.py`**.
