...
- **Layout + workflow contracts** — **`tests/test_ci_validate_layout_sync.py`**: **.sh** / **.ps1** path lists match; **`ci.yml`** **validate** step stays **`bash scripts/ci_validate_layout.sh`** (not re-inlined **`test -f`**); **ci.yml** / **ui-screenshot.yml** keep the expected install, **pytest**, **Qt**, and **wheel** strings; **Python** versions match across workflows; **`test_cursor_rule_github_work_context_points_at_contributing_ci`** guards **`.cursor/rules/github-work-context.mdc`** hub-sync pointers.
