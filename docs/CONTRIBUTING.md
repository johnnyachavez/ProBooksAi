...
- **README `## Desktop app (PySide6)` anchor** — **ROADMAP**, **BACKLOG**, **issues-backlog**, this page's **Other docs**, **README** top **Docs** line, **`review.html`** (Documentation cards + Python section) / **`index.html`** / **`invoice.html`**, **`.github/ISSUE_TEMPLATE/`** bodies, and **`config.yml`** (**Desktop app** contact link) use **`README.md#desktop-app-pyside6`**; renaming that heading updates the slug (**`test_hub_docs_link_readme_desktop_app_section`**, **`test_readme_docs_bar_links_desktop_app_anchor`**, **`test_github_issue_templates_reference_core_docs`**, **`test_static_html_links_readme_desktop_section`**, **`test_review_html_links_readme_web_shell_and_desktop_anchors`**).
- **README `### Excel workbook template (openpyxl)` anchor** — **`bug_report.md`** / **`feature_request.md`** and **`review.html`** (Documentation cards) use **`README.md#excel-workbook-template-openpyxl`**; renaming that Markdown heading updates the slug (**`test_github_issue_templates_reference_core_docs`**, **`test_review_html_links_readme_web_shell_and_desktop_anchors`**, **`test_pr_template_lists_readme_excel_workbook_anchor_checklist`**, **`test_contributing_ci_documents_readme_excel_workbook_anchor_bullet`** in **`test_ci_validate_layout_sync.py`**).
...
| Test module | Guards |
|---|---|
| **`test_pyproject_contract.py`** | **`pyproject.toml`** (name, description, **`[project.urls]` Documentation** → README **`#web-shell-review`**, scripts, **`.[ci]`**, Hatch, **pytest** ini) |
...
| **`test_desktop_main_contract.py`** | **`desktop_app/main.py`** argparse / **Qt** app strings + **`qInstallMessageHandler`** before **`QApplication`** (filters **`QFont::setPointSize`** stderr noise); **`desktop_app/theme.py`** Fusion style + pixel-sized app font ( **`FONT_SIZE_NORMAL`**) before/after **`setStyleSheet`** |
| **`test_probooks_cli_contract.py`** | **`probooks/cli.py`** argparse / epilog |
