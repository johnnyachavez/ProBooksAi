...
| **`test_repo_paths_contract.py`** | **`tests/repo_paths.py`** — shared repo **`Path`** constants stay under **`REPO_ROOT`**; pinned files (**`pyproject.toml`**, **`desktop_app/`**, **`probooks/migrations/`**, **`.github/workflows/ci.yml`**, **`scripts/ci_validate_layout.sh`**, **`integrations/work-context.example.json`**) exist |
