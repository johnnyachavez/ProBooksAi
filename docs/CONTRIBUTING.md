...
- **README `### Default database paths (Windows)`** — Keep **README** default-path bullets aligned with **ROADMAP** implementation snapshot (**Why not one `.db` yet**, **#21**, two filenames); **`test_readme_default_database_paths_notes_two_schemas_and_roadmap`**, **`test_pr_template_lists_readme_default_database_paths_checklist`**, **`test_contributing_ci_documents_readme_default_database_paths_bullet`** in **`test_ci_validate_layout_sync.py`**.
