...
- **`.github/workflows/ci.yml`** — On **push** / **pull_request** to **`main`** / **`master`**: **`validate`** runs **`bash scripts/ci_validate_layout.sh`** (authoritative path list; twin **`scripts/ci_validate_layout.ps1`** for Windows when **`bash`** is not on **`PATH`**; see **README** **Scripts**; both **`require`** **`.gitattributes`** so **`*.yml`**, **`*.yaml`**, and **`*.sh`** stay LF). **`python`** job: **`python-version: "3.12"`**, **`pip install -e ".[ci]"`**, **`python -m pytest`** with **`QT_QPA_PLATFORM=offscreen`**, then **`pip install build && python -m build --wheel`**.
...
| **`test_repo_paths_contract.py`** | **`tests/repo_paths.py`** — shared repo **`Path`** constants stay under **`REPO_ROOT`**; pinned files (**`.gitattributes`**, **`pyproject.toml`**, **`desktop_app/`**, **`probooks/migrations/`**, **`.github/workflows/ci.yml`**, **`scripts/ci_validate_layout.sh`**, **`integrations/work-context.example.json`**) exist |
...
| **`test_integrations_example_contract.py`** | **`integrations/work-context.example.json`** (top-level keys; **`localWorkFiles`** exact set **`index.html`**, **`invoice.html`**, **`review.html`**, **`docs/ROADMAP.md`** (each once); sample **PR** / **issue** rows; **`warnings`** strings must be literals from **`sync-workspace.ps1`**; see **Running Tests** above) |
...
| **`test_desktop_main_contract.py`** | **`desktop_app/main.py`** module doc + argparse (**`--help`** epilog via **`EXCEL_COA_WORKBOOK_ARGPARSE_EPILOG`**) / **Qt** app strings + **`QAction.setStatusTip`** on **File** / **View** / **Edit** / **Tools** / **Help** (incl. disabled items) + **`qInstallMessageHandler`** before **`QApplication`** (filters **`QFont::setPointSize`** stderr noise); **`desktop_app/theme.py`** Fusion style + pixel-sized app font ( **`FONT_SIZE_NORMAL`**) before/after **`setStyleSheet`**; Excel epilog text lives in **`probooks/help_epilog.py`**; **Document Intake** tab **F5** (**`sc_intake_f5`** → **`_refresh_inbox`**) + **`show_document_intake_keyboard_shortcuts_dialog`** (**Help** menu + **`InboxWidget`** **context menu**; **`_document_intake_keyboard_shortcuts_help_text`** lists **View** **Ctrl+1–8** + **Business** / **More tab** cross-links); **`DetailPane`** action buttons **`setToolTip`**; **`desktop_app/register_tab.py`** register grid header **`QSettings`** persistence + shortcuts (**tooltips**, **Help** menu); **`desktop_app/bank_import_tab.py`** **F5** reload + **`show_bank_import_keyboard_shortcuts_dialog`** (**Help** menu + batch/txn / **Manage Bank Accounts** **context menu**; header **Manage** / **Import CSV** **`setToolTip`**; **ManageAccountsDialog** / **ReconciliationPanel** button **`setToolTip`**); **`desktop_app/more_main_tabs_shortcuts.py`** **`show_more_main_tabs_keyboard_shortcuts_dialog`** (**Help** menu + **Keyboard shortcuts…** on **COA** / **Journal** / **Reports** / **Audit** main grids/lists); **`desktop_app/journal_tab.py`** **F5** = **Refresh** + **Export CSV** **`setToolTip`** + **entry list** / **lines** **context menu**; **`desktop_app/extra_tabs.py`** **Business** hub **F5** (**`sc_business_f5`** → **`_refresh_current_subtab`**) + **`show_business_keyboard_shortcuts_dialog`** (**Help** menu + **Rules** / **AR** / **AP** / **Payroll** grid **context menu** + **`_attach_table_copy_row_menu`** modal tables; **`_business_keyboard_shortcuts_help_text`**); **`desktop_app/coa_tab.py`** **F5** + toolbar **`setToolTip`** + grid **context menu**; **`desktop_app/reports_tab.py`** **F5** + run/export **`setToolTip`** + grid **context menu**; **`desktop_app/audit_tab.py`** **F5** + **Export CSV** / **Apply filter** **`setToolTip`** + grid **context menu** |
