...
**Invoice Intake (dispatch sheet):** Desktop **Invoice Intake** maps CSV columns to the live **1 CHAVAN DISPATCH** Google Sheet (file id `112pHWAkiTIk5N31mAUzUuydsSdUh9fVoBSADnZHzlXQ`; [open workbook](https://docs.google.com/spreadsheets/d/112pHWAkiTIk5N31mAUzUuydsSdUh9fVoBSADnZHzlXQ/edit?usp=sharing)). v1 is a CSV export — **not** a live Google token in the app. Live loads are the tab named for the **current calendar year**; if that tab is missing, use the highest 4-digit year tab. Older year tabs are history. Loads headers: `DATE`, `INVOICE`, `DISPATCH`, `DRIVER`, `INVOICE RATE`, `PAY RATE`, `PO / LOAD#`, `BOL #` / `TICKET #`, `QB Inv No.`. Parser drops tax-id / SSN / EIN / DIR / Trucker ID / CA# columns. Tests: `tests/test_dispatch_intake.py`, `tests/test_invoice_intake_panel.py`.
