...
High-level **product** architecture and sequencing. The [implementation snapshot](#implementation-snapshot-repository-2026-04) and [phased roadmap](#phased-roadmap) below map this to modules, tables, and GitHub-tracked phases.

### Core architecture

#### Bank register (CORE)

- **Source of truth** for recorded bank activity in ProBooks+ai (the continuous account ledger).
- **Manual + imported** transactions: CSV/PDF import **feeds** `bank_transactions`; the Register tab is the primary grid for that ledger (including **Add transaction…** for typed lines).
- **Continuous running balance** in register order (per selected bank account).

#### Bank import + AI reconciliation (TOOL)

- Reads **CSV** / **PDF** (AI/OCR is the direction for harder PDFs; text-layer PDF exists today—see snapshot).
- **Extracts** transactions for review before they are committed to the ledger.
- **Compares** statement-side data to the register (period math, batch reconciliation, export).
- **Flags** (product target): **Matched** / **Missing** / **Extra**—implementation and UI will grow toward explicit modes; today's Bank Import + reconciliation panel are steps along that path.

### Current phase

- Build **reconciliation UI**, including **mock or staged extraction** where full AI/OCR is not production-complete.
- **Do not change the register structure** as the primary UX (same tab, same column model): extend via existing `bank_transactions` / batches and adjacent panels rather than replacing the register grid.

### Next phases

1. **Persist manual entries in register** — **in repository:** **Add transaction…** + per-account sentinel batch **`(Manual entry)`** (`BankDatabase.insert_manual_transaction` in `probooksai/bank_import.py`). Further polish (defaults, validation, COA-in-dialog) can follow.
2. **Improve matching logic** (register vs statement lines, dedupe, suggested links).
3. **Add OCR / AI PDF parsing** (see [Phase 7 – PDF / OCR Intake](#phase-7--pdf--ocr-intake)).
4. **Full reconciliation workflow** (locking, reports, batch lifecycle).

### Rules

- **Register is always authoritative** for which bank lines exist in the company file; imports ingest **into** the ledger; reconciliation is a **secondary** control layer.
- **No auto-posting from imports** to the GL—posting stays an explicit user action (e.g. Register **Post selected to GL**).
- **Reconciliation is separate from register editing**: statement periods, tolerances, and **Mark Reconciled** are import/batch concerns; they do not replace the register as the day-to-day ledger UI.
...
**SQLite online backup (regression):** **`probooks/backup.py`** (SQLite online backup API + atomic replace); **`tests/test_backup.py`**; layout contract **`tests/test_probooks_backup_contract.py`** ([CONTRIBUTING.md — Continuous integration](CONTRIBUTING.md#continuous-integration), **Module contracts** / issue [**#28**](https://github.com/johnnyachavez/ProBooksAi/issues/28)).

**Why not one `.db` yet:** The CLI runs ordered migrations in `probooks/migrations/` (`schema_migrations`, `companies`, `import_batches`, …). The desktop `BankDatabase` (`probooksai/bank_import.py`) upgrades via `schema_version` and different tables/columns (`bank_import_batches`, …). Using a single SQLite path for both without a designed schema merge risks incompatible DDL; **#21** is migration design + data path, not renaming the intake file alone. **Bank DDL inventory** (regression): **`tests/test_issue_21_schema_inventory.py`** ([CONTRIBUTING.md — Continuous integration](CONTRIBUTING.md#continuous-integration), **SQLite issue #21** bullet).

**Invoice Intake (dispatch sheet):** Desktop **Invoice Intake** maps CSV columns to the live **1 CHAVAN DISPATCH** Google Sheet (file id `112pHWAkiTIk5N31mAUzUuydsSdUh9fVoBSADnZHzlXQ`; [open workbook](https://docs.google.com/spreadsheets/d/112pHWAkiTIk5N31mAUzUuydsSdUh9fVoBSADnZHzlXQ/edit?usp=sharing)). v1 is a CSV export — **not** a live Google token in the app. Only **new unbilled** loads are queued: the tab named for the **current calendar year** (if missing, the highest 4-digit year tab) and a **blank QB Inv No.** Rows that already have a QB Inv No. are skipped (already invoiced in QuickBooks). History year tabs are not imported. The year tab is the live table: `DATE | INVOICE | DISPATCH | DRIVER | INVOICE RATE | PAY RATE | PO / LOAD# | BOL# | QB Inv No.` (``BOL #`` and older ``TICKET #`` alias to BOL). Parser drops tax-id / SSN / EIN / DIR / Trucker ID / CA# columns. Tests: `tests/test_dispatch_intake.py`, `tests/test_invoice_intake_panel.py`.

**Bank Import → Register Stmt match (mock line reconciliation):** `desktop_app/statement_line_match_panel.py` + `probooksai/statement_line_match.py`; **Run mock extract & compare** classifies Matched / Missing / Extra and syncs results to the Register **Stmt match** column via `RegisterTab.apply_line_match_results_from_import` (reconciliation mode + same bank account selection with int-safe combo matching via **`desktop_app/qt_combo_ids.py`**, also used for **Register** transfer counterparty / manual **Link payment** record ids, **Business** filtered customer/vendor combos, **QSettings** restore of last customer/vendor, and AR/AP **payment bank** combos—with ``start=1`` to skip the leading “none” bank row—in `extra_tabs.py`); `MainWindow` passes `register_tab` and `after_stmt_match_sync` into `BankImportTab` so the UI focuses **Bank register** on success (with a short **status bar** confirmation, then **`QTimer.singleShot`** restores the default company line), and shows a warning when the account cannot be opened; regression / contract coverage in **`tests/test_register_visible_rows.py`**, **`tests/test_qt_combo_ids.py`**, and **`tests/test_desktop_main_contract.py`**.

| Roadmap phases | In-repo status |
|---|---|
| 1–5 | SQLite bank + batches + transactions; CSV import with dedupe; **Bank Import** tab **F5** reload (accounts/batches; restores selected import batch when possible; footer **F5** hint + **Help** → **Bank import shortcuts…** / **Bank register keyboard shortcuts…**; **batch** / **imported-txn** / **Manage Bank Accounts** tables **context menu** **Keyboard shortcuts…** including empty area); reconciliation panel (**Match within** ± tolerance, `QSettings`, drives **Mark Reconciled** + export); Register tab; saved per-account CSV column mapping; bank↔GL cash mapping; **Post selected to GL** (including **split lines → multi-line journal** and **bank-to-bank transfers → two cash GL lines**); **Reports** (**F5** re-run last trial balance / P&L / balance sheet; trial balance, P&L, balance sheet + **Export CSV…** / `financial_reports.write_report_csv`; **sortable** results grid; amount columns via `FloatSortTableItem` / `table_clipboard` (comma decimals, numeric sort, right-aligned); label columns `plain_display_table_item`; **right-click Copy row** / `table_clipboard`); **Journal** viewer (**F5** refresh like **Refresh**; **Export CSV…** / `GLDatabase.journal_export_rows` + `write_journal_export_csv`; **right-click Copy entry line** on the entry list (`table_clipboard.QLIST_PLAIN_TEXT_ROLE` for verbatim text when the list label is Qt-escaped) + **Copy row** on the **sortable** line-detail table (`plain_display_table_item` for account/description/memo; **Debit**/**Credit** via `NumericAmountTableItem` + right align) / `table_clipboard`); COA editor (**F5** refresh; **sortable** grid; account **#** via `_CoaAccountNumberTableItem` in `coa_tab.py` — numeric sort when the number is digits-only, else lexicographic; `UserRole` holds account id; `QTABLE_PLAIN_TEXT_ROLE` for **Copy row**; other read-only cells `plain_display_table_item`; **right-click Copy row** as TSV via `desktop_app/table_clipboard`) |
...
| 17–20 | Register: **filters** (needs receipt / has attachment / missing file / **has or no payment link** / **cleared** / **not cleared** / **batch reconciled** / **batch not reconciled**); **QSettings** scoped by company SQLite path remembers last filter + last bank account + **table header widths** (save on tab hide, restore on show); **Recon** menu (**Register Actions** / **Reconciliation** / **Attachments** / **Transaction Tools** / **Flags**) **`QAction` tips** for add/post/export/cleared/attachments/splits/transfer/link/receipt flags; **F5** + **Ctrl+Shift+G** / **E** / **C** / **U** unchanged with Register focus; on-tab controls: **bank account**, **filter**, **Reconciliation mode**; **Clr** **column header** tooltip (C vs R + Bank Import); **per-row Clr** cell tooltips (cleared, batch reconciled, or neither); **checkbook-style** visible cell borders (`desktop_app/theme.py` **`register_table_style_sheet`**) + word-wrap; **two-line** **Payee** (description + COA/memo sub-line) and **Number** (ref + **DEP**/**PMT**/**XFER**/**TXN**); **Clr** shows **C** when `bank_transactions.cleared` (migration v6; Recon menu + context **Mark cleared** / **Clear cleared**, **Ctrl+Shift+C** / **Ctrl+Shift+U** / **Ctrl+Shift+E** (export CSV) / **Ctrl+Shift+G** (post to GL) / **F5** (refresh) with Register focus, **double-click Clr** toggles; **cleared** updates allowed on **posted** rows) else **R** when the import batch is reconciled; receipt flags, **attach path**, **transfer to** account (with **GL posting** between mapped cash accounts), **Splits** editor, **Link payment** (AR/AP/**payroll run**): **suggested matches** by amount+date, **clear link**, **Match** column + export; **Link** dialog suggested-match list **right-click Copy suggestion line** (`copy_qlistwidget_row_text` + `QLIST_PLAIN_TEXT_ROLE` verbatim label); excludes already-linked records (`suggest_bank_match_candidates`, `unlink_bank_transaction`); **Add transaction…** (`ManualTransactionDialog` in `register_tab.py`) writes `bank_transactions` via **`BankDatabase.insert_manual_transaction`** and a per-account sentinel import batch **`(Manual entry)`** (`get_or_create_manual_entry_batch_id`, `make_manual_entry_fingerprint` in `probooksai/bank_import.py`); regression **`tests/test_bank_import.py`** + contract **`test_register_tab_manual_entry_dialog_and_insert_wiring`**; **Help** menu **Bank register keyboard shortcuts…** (dialog cross-links **Bank import shortcuts…**) + **context menu** **Keyboard shortcuts…** (any grid right-click, including empty area); **right-click** row **Open attachment…** (`desktop_app/open_attachment.open_local_attachment` + `BankDatabase.get_transaction`), **Mark cleared** / **Clear cleared**, **Copy row** (TSV via `table_clipboard`; **COA** column uses embedded combo ``userData`` when set so copied TSV has the raw account string, not Qt-escaped label text; **date** / **payee** / **posted** ref+memo use `plain_display_table_item`; **sortable** grid (reload toggles sorting; txn id on **Date** ``UserRole``); **debit** / **credit** / **balance** via `NumericAmountTableItem` (comma + `QTABLE_PLAIN_TEXT_ROLE`; **balance** stays chronological, not by column sort); **unposted** ref+memo stay raw `QTableWidgetItem` for inline edit); **Match** column **cell tooltips** (**Link payment…**) |
| 21 | CSV import on **worker thread** with **progress dialog + cancel** (`desktop_app/csv_import_worker.py`) |
| 22 | Register **Export CSV** (respects active filter; **Register_cleared** + **Batch_reconciled** columns); Bank Import **Export reconciliation report CSV** per batch; **Journal** / **Reports** export CSV; **Business** AR/AP **customers / invoices / vendors / bills** + **payments** + **payment→invoice/bill allocation** CSV (invoice/bill list export: **visible vs all** when grid filter is set); **Excel COA workbook** via **`generate_workbook.py`** / **`probooksai/generator.py`** (**`openpyxl`** in **`pyproject.toml`**; **[README — Excel workbook template](../README.md#excel-workbook-template-openpyxl)**); shared **Excel** **`--help`** line in **`probooks/help_epilog.py`** (`python -m probooks`, `python -m desktop_app.main`; **README** Desktop section) |
| 23–24 | **COA** / **Journal** / **Reports** / **Audit**: **Help** → **More tab shortcuts (F5)…** (`desktop_app/more_main_tabs_shortcuts.py`); main grid or list **right-click** **Keyboard shortcuts…** including empty area; **Audit log** tab (**F5** refresh like **Refresh**; filter by entity type + id; **Export CSV…** via `probooksai/audit_log.write_audit_csv`; **sortable** log grid (**Entity ID** via `table_clipboard.IntSortTableItem`); **right-click Copy row** (TSV) on the log grid / `table_clipboard`; types include **bank_import_batch** after **Mark reconciled**); **View change history…** context menu on **Register**, **Bank Import** (batch list + **imported transactions**), and **COA** rows (`desktop_app/audit_dialog.py`; **sortable** history grid + **Copy row** (TSV)); **Register** / **Bank Import** (**batch list** + imported txns + **Manage Accounts** list) / **COA** row menus also **Copy row** (TSV); **Bank Import** imported-transaction rows also **Open attachment…** (same helper as Register/AP bill); bank txn + **COA** CRUD; **`--database PATH`** / **`--version`**; **Open/New company**; **Backup** / **Restore** (Issue #28); **File → Copy company database path** (**Ctrl+Alt+P**) copies resolved **`.db`** path to clipboard; **window title** + top **banner** show active company file name on switch; ``&`` escaped via `qt_mnemonic.escape_ampersand_for_qt` in title/banner, **status bar** (ready + company lines with bank CSV/PDF/paste pointer, company path, copied path), skipped-import and roadmap-open warnings, **Open/New company** / **backup** / **restore** path dialogs and OS errors, and **Business** tab (`desktop_app/extra_tabs.py`) rules delete confirmation, rules import/export, invoice PDF save, AR/AP/Payroll/Tax **QMessageBox** paths and exception text, **Tax %** “tax name” row label, filtered entity/bank + payroll GL/employee **QComboBox** labels; **Register** (`register_tab.py`: export basename, posting summary, memo/COA/splits/transfer errors, **Link** dialog “Current link” label, bank-account + per-row **COA** + transfer + **Link payment** combo/list **display** text + **Match** column), **Journal** (`journal_tab.py`: entry-list line text + journal line-detail **Copy row** via `plain_display_table_item`) / **Audit** / **Reports** export paths + **OSError** text + **Reports** tab P&L summary **QLabel** hints, **Bank Import** (`bank_import_tab.py`: **Map CSV Columns** escaped header labels + raw `userData`, **Statement Period & Balances** window title, account picker + **Manage Accounts** GL cash combo **display** vs `userData` + **Manage Accounts** type column / import **batch list** / **TransactionsTable** date/description/ref via `plain_display_table_item`, **Amount** via `NumericAmountTableItem` (numeric sort + `QTABLE_PLAIN_TEXT_ROLE` for copy), PDF/CSV read + PDF error + worker **Import failed** + reconciliation export + **Manage Accounts** delete confirm; **Manage Accounts** + import **batch** lists are **sortable** (row selection uses account/batch id, not row index)), **COA** add/edit **Error** + deactivate **question** (`coa_tab.py`), **`open_attachment`** dialog title + empty message + “file not found” path; **`audit_dialog`** window title + empty-state label; **Create Invoices** / **Invoice Intake** (dispatch spreadsheet; no document-AI inbox); **main window** **menu bar** **`QAction.setStatusTip`** (hover → **status bar** hints); **View** menu **Ctrl+1–8** switch main tabs (Invoices → More); **Help → Product roadmap (local file)** opens `docs/ROADMAP.md` (`desktop_app/local_docs.py` + `QDesktopServices`); desktop **About** + **`probooks.__version__`** track `probooks-ai` metadata (`desktop_app/version.py`); PyInstaller scripts output **ProBooksPlusAi** with **`--paths`** + **probooksai** + **desktop_app** + **`docs`** bundle; **`pyproject.toml`** wheel includes **`probooksai`** + **`probooks`**; CI **wheel build** check; GitHub Actions pytest installs **`.[ci]`** (`pyproject.toml`: pytest + pypdf + PySide6) and sets **`QT_QPA_PLATFORM=offscreen`** so **PySide6** tests run headless (`tests/test_table_clipboard.py`; invoice PDF smoke runs in a **subprocess** on non-Windows so a Qt crash cannot abort pytest, **skipped on Windows** where some Qt builds abort with `0xC0000409`) |
...
#### Implemented: manual register entry (sentinel batch)

- **Add transaction…** (**Recon** → **Register Actions**) opens a dialog (date, signed amount, description, ref, memo) and inserts into `bank_transactions` with the same sign convention as CSV import (positive = inflow, negative = outflow).
- Each bank account has at most one synthetic import batch with filename **`(Manual entry)`**; `batch_id` stays non-null and compatible with Bank Import / reconciliation lists.
- API: `BankDatabase.get_or_create_manual_entry_batch_id`, `insert_manual_transaction`, `make_manual_entry_fingerprint` (`probooksai/bank_import.py`).
...
- User can add new register rows without importing a CSV (manual lines stored like other bank transactions).
...
- **Discoverability:** **`probooks/help_epilog.py`** holds the **Excel COA workbook** argparse epilog string reused by **`python -m probooks --help`** and **`python -m desktop_app.main --help`** (also called out under **README** **`## Desktop app (PySide6)`**).
