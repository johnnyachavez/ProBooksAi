...
def test_business_hub_includes_ai_subtab(qapp: QApplication, tmp_path: Path) -> None:
    ...
    try:
        ...
        # Merged Business hub: Rules=0, Payroll=1, Tax=2, Company=3 (from main),
        # AI appended last so existing saved-index behaviour is unaffected.
        assert bar.count() == 5
        assert bar.tabText(4) == "AI"
        assert isinstance(bar.widget(4), AISettingsTab)
    finally:
        db.close()
